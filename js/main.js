...
function initContactForm() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (!allValid) {
      // Block submission client-side; let the server also re-validate anything that slips through.
      e.preventDefault();
      success.classList.remove('show');
      const firstInvalid = form.querySelector('.is-invalid');
      if (firstInvalid) firstInvalid.focus();
    }
    // If allValid, let the form submit normally to contact.php (PHP handles storage + confirmation).
  });
}
...
function initLoginForm() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (!valid) {
      // Block submission client-side; the server re-validates and checks credentials.
      e.preventDefault();
      if (success) success.classList.remove('show');
    }
    // If valid, let the form submit normally to login.php.
  });
}

/* ---------------------------------------------------------
   3c. Register form validation (username, email, password,
...
function initRegisterForm() {
  ...
  const confirm = form.querySelector('#regConfirmPassword');
  const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

  function validateField(field) {
    let valid = true;
    if (field === username) valid = username.value.trim().length >= 3;
    if (field === email) valid = emailPattern.test(email.value.trim());
    if (field === password) valid = password.value.length >= 6;
    if (field === confirm) valid = confirm.value.length >= 6 && confirm.value === password.value;
    field.classList.toggle('is-invalid', !valid);
    field.classList.toggle('is-valid', valid);
    return valid;
  }
  ...
  form.addEventListener('submit', function (e) {
    const valid = [username, email, password, confirm].map(validateField).every(Boolean);
    if (!valid) {
      e.preventDefault();
      const firstInvalid = form.querySelector('.is-invalid');
      if (firstInvalid) firstInvalid.focus();
    }
    // If valid, let the form submit normally to register.php.
  });
}
